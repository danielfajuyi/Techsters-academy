...
const Navbar = () => {
  ...
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false); // State to handle the burger menu toggle
  ...
  const toggleMobileMenu = () => setIsMobileMenuOpen((prev) => !prev); // Toggle menu for mobile

  return (
    <div className={`flex items-center justify-between px-4 sm:px-10 md:px-14 lg:px-36 border-b border-gray-500 py-4 ${isCourseListPage ? "bg-navbar-bg" : "bg-navbar-bg"}`}
    >
      {/* Logo */}
      <img
        onClick={() => navigate("/")}
      ...
      <div className="md:hidden flex items-center">
        ...
        >
          {isMobileMenuOpen ? (
            <Menu className="text-2xl hidden " />
          ) : (
